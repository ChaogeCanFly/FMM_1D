function [beta, eta_R, eta_T] = FMM_1D_TE_RT_book(epsilon, period, h, lambda, theta, refIndices, N, M, L)
%
% INPUT:
...
for i=1:L
    eps_m(:,i) = fft(epsilon(:,i))/M;
    e1(:,i) = eps_m(1:(2*N+1), i);
    e2(:,i) = eps_m((M-2*N):M, i);
    %we have to renumerate e2 for Toeplitz matrix, because matlab
    %writes eps(m) with negative m in reverse order
    for j=(1:N)
        b = e2(j+1, i);
        e2(j+1) = e2(2*N+1-j+1, i);
        e2(2*N+1-j+1, i)=b;
    end;
    e2(1, i)=e1(1, i);
    eps(:,:,i) = toeplitz(e1(:,i),e2(:,i));
end
